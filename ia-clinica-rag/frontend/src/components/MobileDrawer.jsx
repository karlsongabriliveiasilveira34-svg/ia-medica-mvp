...
function isStudentModeTab(tab) {
  return (
    tab === 'student_notebook' ||
    tab === 'flashcards' ||
    tab === 'quizzes' ||
    tab === 'caderno' ||
    tab === 'library'
...
function DrawerUserCard({ user, highestPct }) {
  return (
...
function DrawerGuestCard({ onLoginClick }) {
  return (
...
export function MobileDrawer({
  ...
  const isStudentMode = isStudentModeTab(activeTab);
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
          {isAuthenticated && user ? (
            <DrawerUserCard user={user} highestPct={highestPct} />
          ) : (
            <DrawerGuestCard onLoginClick={() => handleNavigate('login')} />
          )}
