...
export function SpecialtySelector({ selectedSpecialty, onSelectSpecialty }) {
  ...
  return (
    <div className="media-specialty-selector w-full bg-slate-900/90 border border-slate-800 rounded-2xl p-3 shadow-sm">
      <div className="flex items-center justify-between gap-2 mb-2.5 px-1">
        <span className="hidden sm:flex text-xs font-semibold text-slate-300 items-center gap-1.5">
          <Stethoscope className="w-4 h-4 text-clinical-400" /> Especialidade
        </span>
        <div className="flex items-center gap-2">
          ...
          <span className="max-w-[74vw] truncate text-[11px] text-clinical-400 bg-clinical-500/10 px-2.5 py-1 rounded-full border border-clinical-500/20 font-semibold sm:max-w-none">
            {currentSpec.name}
          </span>
        </div>
      </div>

      <div className="media-specialty-scroll -mx-1 flex flex-nowrap gap-1.5 overflow-x-auto px-1 pb-2">
        {SPECIALTIES.map((spec) => {
          ...
          return (
            <button
              ...
              className={`flex shrink-0 items-center gap-1.5 px-3 py-2 rounded-full text-xs font-medium transition-all ${
                isSelected
                  ? 'bg-[#213f34] text-white shadow-sm border border-[#213f34]'
      ...
      {currentSpec.calculators && currentSpec.calculators.length > 0 && (
        <div className="media-calculators overflow-x-auto pt-2 border-t border-slate-800/80 flex items-center gap-2 text-[11px] text-slate-400 whitespace-nowrap">
          <span className="hidden font-semibold text-slate-300 sm:flex items-center gap-1">
